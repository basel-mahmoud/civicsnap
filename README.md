...
(pothole, broken street light, illegal dumping, graffiti…); Gemini vision reads the image,
...
| AI           | Google Gemini Flash (free tier) via a Supabase Edge Function         |
| Hosting      | Vercel                                                         |
...
+ an Edge Function for Gemini vision). Built in staged commits — see the history.

To enable AI photo classification, set an `GEMINI_API_KEY` secret on the
