...
// Colored teardrop pin with the category emoji, built as a divIcon so we avoid
// shipping image assets and can theme per category.
...
export function categoryIcon(category: string): L.DivIcon {
  ...
  const html = `
    <div style="position:relative;width:34px;height:44px;">
      ...
      <span style="position:absolute;top:5px;left:0;width:34px;text-align:center;font-size:16px;line-height:24px;">${meta.emoji}</span>
    </div>`
