...
// fails fast for `cooldownMs` (protecting a struggling dependency and the user from
// long waits), then allows a single half-open trial before closing again.

export type CircuitState = 'closed' | 'open' | 'half-open'

export class CircuitOpenError extends Error {
  constructor(name: string) {
    super(`${name} is temporarily unavailable`)
...
export class CircuitBreaker {
  ...
  constructor(name = 'service', threshold = 4, cooldownMs = 15_000, now: () => number = Date.now) {
    this.name = name
